...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var spinner: Spinner = findViewById(R.id.CurrencySelector)
        ...
        if (savedInstanceState != null) {
            ...
            con.onSelectedItem(spinner, input, output)
...
class CurrencyOP() {
    ...
    fun checkConvert(input: EditText, spinner: Spinner, output: TextView) {
        var inputDouble = 0.0
        if (input.text.toString() != "") {
            inputDouble = input.text.toString().toDouble()
        }
        Log.d("Convert","Converter")
        if (spinner.selectedItemPosition == 0) { // Dollar -> Euro
            output.text = convert(1.01, inputDouble, "€")
        } else if (spinner.selectedItemPosition == 1) { // Dollar -> Pfund
            output.text = convert(0.87, inputDouble, "£")
        } else if (spinner.selectedItemPosition == 2) { // Euro -> Dollar
            output.text = convert(0.99, inputDouble, "$")
        } else if (spinner.selectedItemPosition == 3) { // Euro -> Pfund
            output.text = convert(0.86, inputDouble, "£")
        } else if (spinner.selectedItemPosition == 4) { // Pfund -> Dollar
            output.text = convert(1.15, inputDouble, "$")
        } else if (spinner.selectedItemPosition == 5) { // Pfund -> Euro
            output.text = convert(1.16, inputDouble, "€")
        } else {
            println(Log.ERROR)
            output.text = convert(1.0, inputDouble, "£") // gibt string zurück
        }
    }
    ...
    fun onSelectedItem(spinner: Spinner, input: EditText, output: TextView) {
        ...
        if (input.text.toString() == "") {
            ...
            checkConvert(input, spinner, output)  //Ausgabe anhand engegebendes im input
        }
    }
}
